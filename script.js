...
// Ucapan bawaan (contoh)
const defaultWishes = [
  { name: 'Nadia Putri',    attend: 'mungkin', message: 'Semoga Allah memberkahi rumah tangga kalian dan dikaruniai keturunan yang sholeh dan sholehah. Aamiin. 🌙' },
  { name: 'Ahmad Fauzi',    attend: 'tidak',   message: 'Maaf tidak bisa hadir, tapi doa terbaik selalu menyertai kalian berdua. Selamat ya Sophia & Rayhan! 🌹' },
  { name: 'Dewi Anggraini', attend: 'hadir',   message: 'Semoga pernikahan kalian dipenuhi cinta dan keberkahan. Langgeng sampai kakek nenek ya! 💕' },
  { name: 'Rizky Pratama',  attend: 'hadir',   message: 'Selamat menempuh hidup baru. Semoga menjadi keluarga yang sakinah, mawaddah, warahmah. Barakallahu lakuma! 🤍' },
];

const STORAGE_KEY = 'sophia_rayhan_wishes';

// Ambil dari localStorage, atau gunakan default jika kosong
function loadWishes() {
  try {
    const saved = localStorage.getItem(STORAGE_KEY);
    return saved ? JSON.parse(saved) : [...defaultWishes];
  } catch { return [...defaultWishes]; }
}

// Simpan ke localStorage
function saveWishes(wishes) {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(wishes)); } catch {}
}

// Render semua kartu ucapan ke DOM
function renderWishes() {
  ...
  wishes.forEach(w => {
    ...
    card.innerHTML = buildCard(w);
    list.appendChild(card);
  });
}

// Buat HTML satu kartu ucapan
function buildCard({ name, attend, message }) {
  ...
  return `
    <div class="wc-header">
      ...
      <div>
        ...
        ${attendMap[attend] || attendMap.mungkin}
      </div>
    </div>
...
function submitWish() {
  ...
  const newWish = { name, attend: selectedAttend || 'mungkin', message };

  // Tambahkan ke depan list & simpan
  const wishes = loadWishes();
  ...
  const list = document.getElementById('wishes-list');
  list.firstChild.scrollIntoView({ behavior: 'smooth', block: 'center' });
}
